...
int main(int argc, char **real_argv, char **envv)
{
	...
	if (!nonotice) {
		snprintf(msgbuf,JOE_MSGBUFSIZE,joe_gettext(_("\\i** Joe's Own Editor v%s ** (%s) ** Copyright %s 2010 **\\i")),VERSION,locale_map->name,(locale_map->type ? "©" : "(C)"));
	...
	if (!idleout) {
		if (!isatty(fileno(stdin)) && modify_logic(maint->curwin->object, ((BW *)maint->curwin->object)->b)) {
			...
			cstart (maint->curwin->object, "/bin/sh", a, NULL, NULL, 0, 1);
		}
	}
