...
HIGHLIGHT_STATE parse(struct high_syntax *syntax,P *line,HIGHLIGHT_STATE h_state)
{
	...
	while((c=pgetc(line))!=NO_MORE_DATA) {
		...
		do {
			...
			if (cmd->delim && (cmd->ignore ? !strcmp(lsaved_s,lbuf) : !strcmp(h_state.saved_s,buf))) {
				cmd = cmd->delim;
	...
	if(!parse_tows(&p, bf)) {
		...
		if(!color) {
			...
			*color_list = color;
			...
			i_printf((char *)joe_gettext(_("%s %d: Class already defined\n")),name,line);
		}
		...
		i_printf((char *)joe_gettext(_("%s %d: Missing class name\n")),name,line);
	}
}
		...
		for (;;) {
			...
			if (!parse_char(&p, ')'))
				...
				if (!parse_ident(&p,bf,sizeof(bf))) {
					...
					while (*param_ptr && (cmp = strcmp(bf,(*param_ptr)->name)) > 0)
						param_ptr = &(*param_ptr)->next;
					if (*param_ptr && !cmp) {
				...
				i_printf((char *)joe_gettext(_("%s %d: Missing )\n")),name,line);
				break;
			}
		}
	}
...
void parse_options(struct high_syntax *syntax,struct high_cmd *cmd,FILE *f,unsigned char *p,int parsing_strings,unsigned char *name,int line)
{
	...
	while (parse_ws(&p,'#'), !parse_ident(&p,bf,sizeof(bf)))
		if(!strcmp(bf,USTR "buffer")) {
			cmd->start_buffering = 1;
		} else if(!strcmp(bf,USTR "hold")) {
			cmd->stop_buffering = 1;
		} else if(!strcmp(bf,USTR "save_c")) {
			cmd->save_c = 1;
		} else if(!strcmp(bf,USTR "save_s")) {
			cmd->save_s = 1;
		} else if(!strcmp(bf,USTR "recolor")) {
			parse_ws(&p,'#');
			if(!parse_char(&p,'=')) {
				parse_ws(&p,'#');
				if(parse_int(&p,&cmd->recolor))
					i_printf((char *)joe_gettext(_("%s %d: Missing value for option\n")),name,line);
			} else
				i_printf((char *)joe_gettext(_("%s %d: Missing value for option\n")),name,line);
		} else if(!strcmp(bf,USTR "call")) {
			parse_ws(&p,'#');
			if(!parse_char(&p,'=')) {
				parse_ws(&p,'#');
				if (!parse_char(&p,'.')) {
					...
					if (!parse_char(&p,'.')) {
						...
						if (parse_ident(&p,bf1,sizeof(bf1)))
							i_printf((char *)joe_gettext(_("%s %d: Missing subroutine name\n")),name,line);
				...
				i_printf((char *)joe_gettext(_("%s %d: Missing value for option\n")),name,line);
		} else if(!strcmp(bf,USTR "return")) {
			cmd->rtn = 1;
		} else if(!strcmp(bf,USTR "reset")) {
			cmd->reset = 1;
		} else if(!parsing_strings && (!strcmp(bf,USTR "strings") || !strcmp(bf,USTR "istrings"))) {
			if (bf[0]=='i')
				cmd->ignore = 1;
				...
				if (*p) {
					...
					if(parse_string(&p,bf,sizeof(bf)) >= 0) {
						...
						if(!parse_ident(&p,bf1,sizeof(bf1))) {
							...
							i_printf((char *)joe_gettext(_("%s %d: Missing state name\n")),name,line);
					} else
						i_printf((char *)joe_gettext(_("%s %d: Missing string\n")),name,line);
				}
			}
		} else if(!strcmp(bf,USTR "noeat")) {
			cmd->noeat = 1;
		} else if(!strcmp(bf,USTR "mark")) {
			cmd->start_mark = 1;
		} else if(!strcmp(bf,USTR "markend")) {
			cmd->stop_mark = 1;
		} else if(!strcmp(bf,USTR "recolormark")) {
			cmd->recolor_mark = 1;
		} else
			i_printf((char *)joe_gettext(_("%s %d: Unknown option\n")),name,line);
}
				...
				if (!strcmp(bf, USTR "ifdef")) {
					...
					if (!stack || !stack->ignore) {
						...
						if (!parse_ident(&p, bf, sizeof(bf))) {
							...
							for (param = syntax->params; param; param = param->next)
								if (!strcmp(param->name, bf)) {
									st->ignore = 0;
									break;
								}
							st->skip = 0;
						} else {
							i_printf((char *)joe_gettext(_("%s %d: missing parameter for ifdef\n")),name,line);
						}
					}
					stack = st;
				} else if (!strcmp(bf, USTR "else")) {
					if (stack && !stack->else_part) {
						stack->else_part = 1;
						if (!stack->skip)
							stack->ignore = !stack->ignore;
					} else
						i_printf((char *)joe_gettext(_("%s %d: else with no matching if\n")),name,line);
				} else if (!strcmp(bf, USTR "endif")) {
					if (stack) {
					...
					if (parse_ident(&p, bf, sizeof(bf))) {
						...
						if (!stack || !stack->ignore) {
							inside_subr = 1;
							this_one = 0;
							if (syntax->subr && !strcmp(bf, syntax->subr))
								this_one = 1;
						}
					}
				} else if (!strcmp(bf, USTR "end")) {
					if (!stack || !stack->ignore) {
						this_one = 0;
						inside_subr = 0;
					}
				} else {
					i_printf((char *)joe_gettext(_("%s %d: Unknown control statement\n")),name,line);
				}
			} else {
				i_printf((char *)joe_gettext(_("%s %d: Missing control statement name\n")),name,line);
			}
			...
			if(!parse_ident(&p, bf, sizeof(bf))) {
				...
				if(!parse_tows(&p,bf)) {
					...
					if(color)
						...
						state->color=0;
						i_printf((char *)joe_gettext(_("%s %d: Unknown class\n")),name,line);
					}
				} else
					i_printf((char *)joe_gettext(_("%s %d: Missing color for state definition\n")),name,line);
			} else
				i_printf((char *)joe_gettext(_("%s %d: Missing state name\n")),name,line);
		} else if(!parse_char(&p, '-')) {
			...
			if (!c) {
				...
				if (state) {
					...
					if(!parse_field(&p, USTR "*")) {
						...
						if(c < 0)
							i_printf((char *)joe_gettext(_("%s %d: Bad string\n")),name,line);
						else {
							int z;
							int first, second;
							unsigned char *t = bf;
					...
					if(!parse_ident(&p,bf,sizeof(bf))) {
						...
						i_printf((char *)joe_gettext(_("%s %d: Missing jump\n")),name,line);
				} else
					i_printf((char *)joe_gettext(_("%s %d: No state\n")),name,line);
			} else
				i_printf((char *)joe_gettext(_("%s %d: Unknown character\n")),name,line);
		}
	}
...
int syntax_match(struct high_syntax *syntax,unsigned char *name,unsigned char *subr,struct high_param *params)
{
	...
	if (!syntax->subr ^ !subr)
		return 0;
	if (subr && strcmp(syntax->subr,subr))
		return 0;
	syntax_params = syntax->params;
	while (syntax_params && params) {
		if (strcmp(syntax_params->name,params->name))
			return 0;
